...
public class ChatUser extends AppCompatActivity {
    ...
    CustomeUserAdapter customeUserAdapter;
    ...
    public void retrieveMessageTask(){
        new RetrieveMessageTask(getApplicationContext(), false) {
            @Override
            public void OnApiResult(int responseCode, int statusCode, String failReason) {
                if (statusCode == 1){
                    ...
                    Toast.makeText(ChatUser.this, "errorMsg", Toast.LENGTH_SHORT).show();
                }
                else{
                    Toast.makeText(ChatUser.this, "errorMsg", Toast.LENGTH_SHORT).show();
                }
    ...
    private void init(final MessageInfo myDataset){
        ...
        customeUserAdapter = new CustomeUserAdapter(ChatUser.this,"737844c5-de02-11e6-83b5-42010af00aa4",listItems);
        recyclerView.setAdapter(customeUserAdapter);
            ...
            @Override public void onScrolledToEnd(final int firstVisibleItemPosition) {
                ...
                if (alreadyLoad){
                    if (pageCount <= totalPage) {
                        if (!loading){
                            Log.e("checkLoad", "not show progress " + firstVisibleItemPosition + " && " + customeUserAdapter.getItemCount());
                            ...
                            refreshView(recyclerView, new CustomeAdapter(ChatUser.this,"737844c5-de02-11e6-83b5-42010af00aa4",itemsLocal), firstVisibleItemPosition);
                            /*} else {
    ...
    private void simulateLoading() {
        //new simuLoading(ChatUser.this).execute();
    ...
    /*public static class simuLoading extends AsyncTask<Void, Void, Void> {
        private final WeakReference<ChatUser> mActivityRef;
        public simuLoading(ChatUser activity){
    ...
    public void scrollToBottom() {
        if (customeUserAdapter!=null){
            if (customeUserAdapter.getItemCount() > 1){
                try {
